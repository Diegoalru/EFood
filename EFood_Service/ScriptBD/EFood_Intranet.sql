...
BEGIN
    IF(@TIPO_TARJETA = 0)
        BEGIN
            SET @TIPO_TARJETA = NULL;
        END
    INSERT INTO PEDIDO
    (TRANSACCION, PROCESADOR, TIPO_TARJETA, DESCUENTO, ESTADO)
    VALUES
    (@TRANSACCION, @PROCESADOR, @TIPO_TARJETA, (SELECT CODE FROM DESCUENTO WHERE CODIGO = @DESCUENTO ), @ESTADO);
end
go
...
CREATE OR ALTER FUNCTION dbo.RETORNA_VALOR_DESCUENTO_CLIENTE
(
    @CODIGO NVARCHAR(6)
)
RETURNS INT
...
BEGIN
    ...
    IF(EXISTS(SELECT MONTO FROM PRECIO WHERE CODE = @CODE))
    BEGIN
        SELECT @RESULT = MONTO FROM PRECIO WHERE CODE = @CODE;  
    END
    ELSE
        BEGIN
            SET @RESULT = -1;
...
FROM
    TIPO_TARJETA AS TT
        INNER JOIN TARJETA_PROCESADOR TP on TT.CODE = TP.TIPO
        INNER JOIN PROCESADOR_PAGO PP on TP.PROCESADOR = PP.CODE
...
(
    ...
    ,CONSTRAINT FK_CLIENTE_TRANSACCION FOREIGN KEY (TRANSACCION) REFERENCES PEDIDO(TRANSACCION)
    ,CONSTRAINT UQ_CLIENTE_TRANSACCION UNIQUE (TRANSACCION)  
);

DROP INDEX IF EXISTS PEDIDO.IDX_PEDIDO_TRANSACCION;
CREATE INDEX IDX_PEDIDO_TRANSACCION ON PEDIDO ( TRANSACCION );
...
(
    ...
    RETURNS TABLE
        ...
        SELECT
            PRC.CODE AS CODE
            ,CONCAT(TP.TIPO, ' - ', PRC.MONTO) AS DATOS
            ,PRC.MONTO AS MONTO
        FROM
            PRECIO AS PRC
